...
N = 12;                     % total number of reference cells
nr = N/2;                   % number of reference cells on one side
ng = 2;                     % number of guard cells on each side
...
%     fft_winframe_mag2(:,(frameLen/2)-19:(frameLen/2)+21) = 0;
    ...
    if disp
        ...
        title({['SOCA-CFAR Range-Doppler map (frame: ', num2str(i), ')'],['Dataset: ', dataset]})
    ...
    observedRangeBin_det_mat(i, :) = t_soca(observedRow,:) < fft_winframe_mag2(observedRow,:);
...
title({'SOCA-CFAR Target Range vs Time', ['(Dataset: ', dataset, ')']})
xlabel('Time (s)')
ylabel('Range (m)')
...
title({['SOCA-CFAR Detections on Range Bin ', num2str(observedRangeBin),...
...
outputpathpart = fullfile('outputs', class, dataset);

saveas(gcf, [outputpathpart, '\spectro'], 'fig')
