...
ng = 0;                     % number of guard cells on each side
...
%     fft_winframe_mag2(:,(frameLen/2)-19:(frameLen/2)+21) = 0;
    ...
    if disp
        ...
        title({['OS-CFAR Range-Doppler map (frame: ', num2str(i), ')'],['Dataset: ', dataset]})
    ...
    observedRangeBin_det_mat(i, :) = t_os(observedRow,:) < fft_winframe_mag2(observedRow,:);
...
title({'OS-CFAR Target Range vs Time', ['(Dataset: ', dataset, ')']})
xlabel('Time (s)')
ylabel('Range (m)')
...
title({['OS-CFAR Detections on Range Bin ', num2str(observedRangeBin),...
