...
% data
% -------------------------------------------------------------------------
class = 'walking';          % class of dataset
dataset = 'walking1';       % dataset being run
...
data_files = dir(fullfile(folder, '*.brd'));    % load data files
...
alpha = (pfa_set.^(-1/N))-1;    % ca cfar constant
...
first = 1 + ng + nr;                    % first column to threshold
last = nfft - nr - ng;                  % last column to threshold
...
%     fft_winframe_mag2(:,(frameLen/2)-19:(frameLen/2)+21) = 0;
    ...
    if disp
        ...
        title({['Range-Doppler map (frame: ', num2str(i), ')'],['Dataset: ', dataset]})
    ...
    tca = zeros(size(fft_winframe_mag2));   % initialise threshold array for current frame
    
    % set threshold and count number of detections
    for j = 1:nrow          % rows
        numDet = 0;         % number of detections for range bin in current frame
        
        for k = first:last    % 'first' to 'last' column
            % sum reference cells
            gca = sum(fft_winframe_mag2(j,(k-ng-nr):(k-ng-1))) + sum(fft_winframe_mag2(j,(k+ng+1):(k+ng+nr)));
            % set threshold for cell under test
            tca(j, k) = gca*alpha;

            % check for detection
            if tca(j,k) < fft_winframe_mag2(j,k)
                numDet = numDet+1;
                if disp
    ...
    if disp
        hold off
        outputpath = fullfile('outputs', class, dataset, ['freq_range_doppler_', num2str(i), '.png']);
...
title({'Target Range vs Time', ['(Dataset: ', dataset, ')']})
xlabel('Time (s)')
ylabel('Range (m)')

outputpath = fullfile('outputs', class, dataset, ['freq_location_vs_time.png']);
set(gcf,'PaperPosition',[0 0 16 10])
print(gcf, '-dpng', outputpath);
